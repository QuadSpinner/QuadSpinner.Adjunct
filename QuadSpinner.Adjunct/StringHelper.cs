...
public static class StringHelper
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)] internal static bool IsEmpty(this string s) => string.IsNullOrEmpty(s) || string.IsNullOrWhiteSpace(s);

    /// <summary>Checks if the string is not null or empty.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)] internal static bool NotEmpty(this string s) => !string.IsNullOrEmpty(s);

    /// <summary>Combines an array of strings into a single string with line breaks.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)] internal static string ToLines(this string[] lines) => string.Join(Environment.NewLine, lines.Select(line => line));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)] internal static bool ContainsIgnoreCase(this string s, string criteria) => s.IndexOf(criteria, StringComparison.CurrentCultureIgnoreCase) > -1;

    /// <summary>Checks if the string equals the given value, ignoring case.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)] internal static bool EqualsIgnoreCase(this string s, string criteria) => string.Equals(s, criteria, StringComparison.CurrentCultureIgnoreCase);
}